...
def plot_result(results, news_site, keyword, start_date="2018-01-01"):
    ...
    axs.bar(unique_months, article_counts, width=20, color="#007acc")
    axs.set_title(f"Number of Articles Published Per Month for {news_site} on keyword {keyword}", fontsize=14)
    axs.set_ylabel("Article Count", fontsize=12)
    axs.set_xlabel("Month", fontsize=12)
    axs.grid(True)
    axs.xaxis.set_major_locator(four_months)
    axs.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m')) # Format as "YYYY-MM"
    axs.tick_params(axis="x", rotation=45)
    axs.yaxis.set_major_locator(MaxNLocator(integer=True))  # Ensure y-axis labels are integers
    axs.set_xlim(unique_months[0] - timedelta(days=15), unique_months[-1] + timedelta(days=15))
